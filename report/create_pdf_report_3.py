...
def new_section(pdf: FPDF, text: str, image_list: list = None, table_name: str = None, new_x: int = XPos.LMARGIN, new_y: int = YPos.NEXT, **kwargs):
    pdf.multi_cell(
        w=pdf.epw,
        h=pdf.font_size,
        txt=text,
...
section_and_content = {'Results summary':
                           {'Validation test performance':
                            ...
                            'Validation performance compared to version used in calibration':
                           ...
                           {'Incidence by age':
                            ...
                            'Infectiousness to vectors':
                                ['Each of the below plot panels corresponds to a site. Within a plot panel, each row corresponds to an age group and each column corresponds to the month when sampling occurred.\n'
                                 ...
                                 ['_plots/%s' % ff for ff in os.listdir('./_plots') if re.match(r'site_compare_infectiousness.*\.', ff)],
                                 None],
                            ...
                            'Asexual parasite density by age':
                                ['Each plot panel corresponds to a site.',
                                 ['_plots/%s' % ff for ff in os.listdir('./_plots') if re.match(r'site_compare_par_dens_age.*\.', ff)],
                             None],
                            'Gametocyte density by age':
                                ['Each plot panel corresponds to a site.',
                                 ['_plots/%s' % ff for ff in os.listdir('./_plots') if re.match(r'site_compare_gamet_dens_age_.*\.', ff)],
                             None]},
                       'Quantitative comparison of reference data and matched simulations':
                           {'Incidence by age':
                                ['Below, the plots show the correlation between simulation and reference points (left plot) and at the correlation between the slopes of the lines between data points (right plot).',
