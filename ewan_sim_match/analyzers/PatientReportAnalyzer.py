...
class PatientAnalyzer(IAnalyzer):
    ...
    def map(self, data, simulation: Simulation):
        ...
        logger.info('Parsing channels %s for %d patients and %d time steps',
             self.channels, npatients, ntsteps)
        ...
        patient_info_temp = np.empty((ntsteps, 3+len(self.channels)), dtype='float')
        ...
        for i, p in enumerate(patients):
            ...
            pos_first_data = max(birthdays[i], 0)  # get day of simulation this patient appears in simulation
            # calculate age of patient on each day of simulation
            patient_info[pos_first_data:, 2] = np.arange(start=initial_ages[i], stop=(initial_ages[i] + ntsteps - birthdays[i] - 1))
